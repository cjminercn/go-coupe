...
import (
	...
	"github.com/cjminercn/go-coupe/common/registrar"
...
func (self *testjethre) UnlockAccount(acc []byte) bool {
	err := self.cjminercn.AccountManager().Unlock(common.BytesToAddress(acc), "")
	if err != nil {
...
func (self *testjethre) ConfirmTransaction(tx string) bool {
	if self.cjminercn.NatSpec {
		self.lastConfirm = natspec.GetNotice(self.xeth, tx, self.client)
	}
	return true
}

func testJEthRE(t *testing.T) (string, *testjethre, *eth.cjminercn) {
	return testREPL(t, nil)
}

func testREPL(t *testing.T, config func(*eth.Config)) (string, *testjethre, *eth.cjminercn) {
	...
	assetPath := filepath.Join(os.Getenv("GOPATH"), "src", "github.com", "cjminercn", "go-coupe", "cmd", "mist", "assets", "ext")
	client := comms.NewInProcClient(codec.JSON)
	tf := &testjethre{client: cjminercn.HTTPClient()}
	repl := newJSRE(cjminercn, assetPath, "", client, false, tf)
	tf.jsre = repl
	return tmp, tf, cjminercn
}

func TestNodeInfo(t *testing.T) {
	t.Skip("broken after p2p update")
	tmp, repl, cjminercn := testJEthRE(t)
	if err := cjminercn.Start(); err != nil {
		t.Fatalf("error starting cjminercn: %v", err)
	}
	defer cjminercn.Stop()
...
func TestRPC(t *testing.T) {
	tmp, repl, cjminercn := testJEthRE(t)
	if err := cjminercn.Start(); err != nil {
		t.Errorf("error starting cjminercn: %v", err)
		return
	}
	defer cjminercn.Stop()
	defer os.RemoveAll(tmp)
...
func TestContract(t *testing.T) {
	...
	tmp, repl, cjminercn := testREPL(t, func(conf *eth.Config) {
...
func pendingTransactions(repl *testjethre, t *testing.T) (txc int64, err error) {
	txs := repl.cjminercn.TxPool().GetTransactions()
...
func processTxs(repl *testjethre, t *testing.T, expTxc int) bool {
	...
	err = repl.cjminercn.StartMining(runtime.NumCPU(), "")
	if err != nil {
		t.Errorf("unexpected error mining: %v", err)
		return false
	}
	defer repl.cjminercn.StopMining()
