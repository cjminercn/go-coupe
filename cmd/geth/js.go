...
import (
	...
	re "github.com/cjminercn/go-coupe/jsre"
	"github.com/cjminercn/go-coupe/rpc"
...
func newLightweightJSRE(docRoot string, client comms.cjminercnClient, datadir string, interactive bool) *jsre {
	js := &jsre{ps1: "> "}
...
func newJSRE(cjminercn *eth.cjminercn, docRoot, corsDomain string, client comms.cjminercnClient, interactive bool, f xeth.Frontend) *jsre {
	js := &jsre{cjminercn: cjminercn, ps1: "> "}
	// set default cors domain used by startRpc from CLI flag
	js.corsDomain = corsDomain
	if f == nil {
		f = js
	}
	js.xeth = xeth.New(cjminercn, f)
	js.wait = js.xeth.UpdateState()
	js.client = client
	if clt, ok := js.client.(*comms.InProcClient); ok {
		if offeredApis, err := api.ParseApiString(shared.AllApis, codec.JSON, js.xeth, cjminercn); err == nil {
	...
	if !liner.TerminalSupported() || !interactive {
		...
		lr := liner.NewLiner()
		js.withHistory(cjminercn.DataDir, func(hist *os.File) { lr.ReadHistory(hist) })
		lr.SetCtrlCAborts(true)
		js.loadAutoCompletion()
		lr.SetWordCompleter(apiWordCompleter)
		lr.SetTabCompletionStyle(liner.TabPrints)
		js.prompter = lr
		js.atexit = func() {
			js.withHistory(cjminercn.DataDir, func(hist *os.File) { hist.Truncate(0); lr.WriteHistory(hist) })
			lr.Close()
			close(js.wait)
		}
	}
...
func (js *jsre) apiBindings(f xeth.Frontend) error {
	...
	apiImpl, err := api.ParseApiString(strings.Join(apiNames, ","), codec.JSON, js.xeth, js.cjminercn)
	if err != nil {
	...
	err = js.re.Compile("cjminercn.js", re.Web3_JS)
	if err != nil {
...
func (self *jsre) ConfirmTransaction(tx string) bool {
	if self.cjminercn.NatSpec {
		notice := natspec.GetNotice(self.xeth, tx, self.cjminercn.HTTPClient())
		fmt.Println(notice)
...
func (self *jsre) UnlockAccount(addr []byte) bool {
	...
	// TODO: allow retry
	if err := self.cjminercn.AccountManager().Unlock(common.BytesToAddress(addr), pass); err != nil {
		return false
	} else {
