...
// Package utils contains internal helper functions for go-coupe commands.
package utils
...
func Startcjminercn(cjminercn *eth.cjminercn) {
	glog.V(logger.Info).Infoln("Starting", cjminercn.Name())
	if err := cjminercn.Start(); err != nil {
		Fatalf("Error starting cjminercn: %v", err)
	}
	go func() {
