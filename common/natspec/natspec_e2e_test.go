...
import (
	...
	xe "github.com/cjminercn/go-coupe/xeth"
)
...
func (self *testFrontend) UnlockAccount(acc []byte) bool {
	self.cjminercn.AccountManager().Unlock(common.BytesToAddress(acc), "password")
	return true
}
...
func testEth(t *testing.T) (cjminercn *eth.cjminercn, err error) {
...
func testInit(t *testing.T) (self *testFrontend) {
	// initialise and start minimal cjminercn stack
	cjminercn, err := testEth(t)
	if err != nil {
		t.Errorf("error creating cjminercn: %v", err)
		return
	}
	err = cjminercn.Start()
	if err != nil {
	...
	// mock frontend
	self = &testFrontend{t: t, cjminercn: cjminercn}
	self.xeth = xe.New(cjminercn, self)
	self.wait = self.xeth.UpdateState()
	addr, _ := self.cjminercn.Etherbase()
...
func TestNatspecE2E(t *testing.T) {
	...
	tf := testInit(t)
	defer tf.cjminercn.Stop()
	addr, _ := tf.cjminercn.Etherbase()
