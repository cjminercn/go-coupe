...
// Package xeth is the interface to all cjminercn functionality.
package xeth
...
import (
	...
	"github.com/cjminercn/go-coupe/eth/filters"
...
func NewTest(eth *eth.cjminercn, frontend Frontend) *XEth {
...
func New(cjminercn *eth.cjminercn, frontend Frontend) *XEth {
	xeth := &XEth{
		backend:          cjminercn,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(cjminercn.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              eth.NewGasPriceOracle(cjminercn),
	}
	if cjminercn.Whisper() != nil {
		xeth.whisper = NewWhisper(cjminercn.Whisper())
	}
	cjminercn.Miner().Register(xeth.agent)
	if frontend == nil {
