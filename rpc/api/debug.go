...
func NewDebugApi(xeth *xeth.XEth, cjminercn *eth.cjminercn, coder codec.Codec) *debugApi {
	return &debugApi{
		xeth:     xeth,
		cjminercn: cjminercn,
...
func (self *debugApi) DumpBlock(req *shared.Request) (interface{}, error) {
	...
	stateDb, err := state.New(block.Root(), self.cjminercn.ChainDb())
	if err != nil {
		return nil, err
	}
...
func (self *debugApi) SetHead(req *shared.Request) (interface{}, error) {
	...
	self.cjminercn.BlockChain().SetHead(uint64(args.BlockNumber))
...
func (self *debugApi) ProcessBlock(req *shared.Request) (interface{}, error) {
	...
	var (
		blockchain = self.cjminercn.BlockChain()
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), self.cjminercn.ChainDb())
	if err != nil {
		return false, err
	}
