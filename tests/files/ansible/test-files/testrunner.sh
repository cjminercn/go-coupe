...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-cjminercn/build/test/createRandomStateTest\" cjminercn/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pycjminercn
	OUTPUT_PYTHON="$(docker run --rm cjminercn/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm cjminercn/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm cjminercn/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
